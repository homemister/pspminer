...
/*int fulltest(const unsigned char *hash, const unsigned char *target)
{
	...
	//return rc;	/* FIXME: return rc; */
//}
static inline u32 e0(u32 word) { 
    u32 result; 
    __asm__ volatile (
	   "ROTR      $a0,   %1, 2\n"
	   "ROTR      $a1,   %1, 13\n"
...
uint32_t scanhash_c(int thr_id, const unsigned char *midstate, unsigned char *data,
	        ...
	        uint32_t max_nonce, unsigned long *hashes_done, uint32_t n, unsigned long stat_ctr)
{
	...
	while (1) {
		...
		runhash(hash1, data, midstate); //424 cycles per function
		runhash(hash, hash1, sha256_init_state); //424 cycles per function

		//stat_ctr++;
		
		if ((hash32[7] == 0x0)) { // && fulltest(hash, target)
			*hashes_done = (unsigned long)n; //stat_ctr
			return 1;
		}
		if(s==1000000){
			*hashes_done = (unsigned long)n; //stat_ctr
